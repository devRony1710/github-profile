...
export const GithubSearchBarHeader: FC<GithubSearchBarHeaderProps> = ({ handleBlurInput, handleFocusInput, isInputFocus, handleOnChange, isSearchingResults, userData, userImage }) => {
      ...
      <div className={clsx('relative flex items-center gap-4 w-[400px] h-[56px] rounded-lg bg-medium-grey px-4 py-2', isInputFocus ? 'border-2 border-custom-blue' : '')}>
        ...
        {isSearchingResults && (
          <div className="absolute top-16 -left-1 w-[400px] h-[100px] py-3 px-2 bg-main rounded-2xl">
            <UserDropdownCompleteState userImage={userImage} />
          </div>
        )}
      </div>
    </section>
